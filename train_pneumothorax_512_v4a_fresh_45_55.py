...
SMOKE_TEST = os.environ.get("V4A_SMOKE_TEST", "0") == "1"
...
LAST_CHECKPOINT_PATH = (
    ...
    / (
        ...
        if SMOKE_TEST
        else "pneumothorax_512_v4a_fresh_45_55_smoke_last.pth" if SMOKE_TEST else "pneumothorax_512_v4a_fresh_45_55_last.pth"
    )
)
BEST_CHECKPOINT_PATH = (
    ...
    / (
        ...
        if SMOKE_TEST
        else "pneumothorax_512_v4a_fresh_45_55_smoke_best.pth" if SMOKE_TEST else "pneumothorax_512_v4a_fresh_45_55_best.pth"
    )
)
TRAINING_STAGE = (
...
def dataset_labels(dataset):
    """Return one image-level pneumothorax label per dataset row."""
    return [
...
class RotatingFortyFiveFiftyFiveSampler(Sampler):
    """
    Use every positive image once per epoch and enough unique negative
    images to produce an approximately 45% positive / 55% negative
    training mixture.

    Negatives are arranged in one deterministic shuffled cycle. Each
    epoch advances through that cycle, so all negatives are seen before
    the cycle repeats.
    """
    ...
    def __init__(
        self,
        dataset,
        positive_fraction=TRAINING_POSITIVE_FRACTION,
        seed=RANDOM_SEED,
    ):
        ...
        if not self.positive_indices:
            raise RuntimeError("Training dataset contains no positives.")

        if not negative_indices:
            raise RuntimeError("Training dataset contains no negatives.")

        self.negative_count_per_epoch = math.ceil(
            len(self.positive_indices)
            * (1.0 - positive_fraction)
            / positive_fraction
        )

        if self.negative_count_per_epoch > len(negative_indices):
            raise RuntimeError(
                "The requested training mixture requires more unique "
                "negative images than are available."
            )

        negative_generator = torch.Generator()
        negative_generator.manual_seed(seed + 7001)
        ...
        self.negative_cycle = [
            ...
            for position in negative_order
        ]
    ...
    def negative_indices_for_epoch(self):
        negative_total = len(self.negative_cycle)

        start = (
            (self.epoch - 1) * self.negative_count_per_epoch
        ) % negative_total

        return [
            self.negative_cycle[
                (start + offset) % negative_total
            ]
            for offset in range(self.negative_count_per_epoch)
        ]

    def indices_for_epoch(self):
        indices = (
            list(self.positive_indices)
            + self.negative_indices_for_epoch()
        )

        epoch_generator = torch.Generator()
        epoch_generator.manual_seed(self.seed + self.epoch)
...
def create_training_sampler(dataset):
    return RotatingFortyFiveFiftyFiveSampler(
        dataset=dataset,
        positive_fraction=TRAINING_POSITIVE_FRACTION,
        seed=RANDOM_SEED,
    )
    ...
    torch.save(
        {
            ...
            "training_sampling": (
                "45% positive / 55% negative with rotating "
...
def main():
    ...
    print("V4A fresh 512 x 512 pneumothorax training")
    print("------------------------------------------------")
    ...
    if RESUME_IF_AVAILABLE and LAST_CHECKPOINT_PATH.is_file():
        print(f"Resuming V4A checkpoint: {LAST_CHECKPOINT_PATH}")
        ...
        best_validation_score = resume_state["best_validation_score"]
        ...
        print(
            "Fresh V4A initialization: ImageNet-pretrained "
    ...
    for epoch in range(start_epoch, MAX_EPOCHS + 1):
        ...
        planned_indices = list(iter(training_loader.sampler))
        planned_positive_count = sum(
            int(training_loader.dataset.rows[index]["HasPneumothorax"])
            for index in planned_indices
        )
        planned_negative_count = (
            len(planned_indices) - planned_positive_count
        )
        planned_positive_fraction = (
            planned_positive_count / len(planned_indices)
        )

        print(f"\nEpoch {epoch}/{MAX_EPOCHS}")
        print(
            "  Planned rotating sample: "
            f"{planned_positive_count:,} positive + "
            f"{planned_negative_count:,} unique negative = "
            f"{len(planned_indices):,} images "
