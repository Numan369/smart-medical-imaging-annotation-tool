...
from pneumothorax_loss import BCEDiceLoss
...
# Train the frozen-encoder stage for one complete epoch.
NUMBER_OF_EPOCHS = 1
...
LAST_CHECKPOINT_PATH = (
    CHECKPOINT_DIRECTORY / "decoder_stage_last.pth"
)
BEST_CHECKPOINT_PATH = (
    CHECKPOINT_DIRECTORY / "decoder_stage_best.pth"
)
...
def calculate_dice_scores(
    ...
    """Calculate one binary Dice score per image."""
...
def train_one_epoch(
    ...
    # Keep the frozen encoder, including its BatchNorm
    # layers, in evaluation mode.
...
def save_checkpoint(
    ...
    """Save everything needed to continue training."""
...
def load_checkpoint(
    ...
    """Restore a decoder-training checkpoint."""
    ...
    if checkpoint.get("training_stage") != "decoder_only":
        raise ValueError(
...
def print_epoch_summary(
    ...
    print(
        f"  Validation positive-case Dice: "
...
def main():
    ...
    validation_loader = find_validation_loader(
        data_loaders
    )

    print(
        f"Training batches available: "
    ...
    if start_epoch > NUMBER_OF_EPOCHS:
        print(
            "\nThe requested decoder-training epochs "
    ...
    try:
        ...
        # Preserve partial parameter updates. The interrupted
        # epoch will start again when the script is rerun.
        save_checkpoint(
            path=LAST_CHECKPOINT_PATH,
    ...
    print("\nDecoder-training stage finished.")
    print(
