...
EXPERIMENT_NAME = "v4b_fixed_45_55"
RANDOM_SEED = 42
IMAGE_SIZE = 512
BATCH_SIZE = 2
...
# The fixed training manifest contains approximately 45% positive and 55%
# negative images. Unlike V4A, the negative image identities do not rotate.
TRAINING_POSITIVE_FRACTION = 0.45
...
SMOKE_TEST = os.environ.get("V4B_SMOKE_TEST", "0") == "1"
MAX_EPOCHS = 1 if SMOKE_TEST else 20
...
V4B_DATA_DIRECTORY = (
    DRIVE_PROJECT_ROOT / "prepared_data" / "v4b_fixed_45_55"
)
TRAIN_CSV = V4B_DATA_DIRECTORY / "v4b_fixed_train.csv"
VALIDATION_CSV = V4B_DATA_DIRECTORY / "v4b_fixed_validation.csv"
TEST_CSV = V4B_DATA_DIRECTORY / "v4b_fixed_test.csv"

CHECKPOINT_DIRECTORY = (
    DRIVE_PROJECT_ROOT / "checkpoints" / EXPERIMENT_NAME
)
LAST_CHECKPOINT_PATH = (
    CHECKPOINT_DIRECTORY
    / (
        "pneumothorax_512_v4b_fixed_45_55_smoke_last.pth"
        if SMOKE_TEST
        else "pneumothorax_512_v4b_fixed_45_55_last.pth"
    )
)
BEST_CHECKPOINT_PATH = (
    CHECKPOINT_DIRECTORY
    / (
        "pneumothorax_512_v4b_fixed_45_55_smoke_best.pth"
        if SMOKE_TEST
        else "pneumothorax_512_v4b_fixed_45_55_best.pth"
    )
)
TRAINING_STAGE = (
    "pneumothorax_512_v4b_fixed_45_55_smoke_test"
    if SMOKE_TEST
    else "pneumothorax_512_v4b_fixed_45_55_training"
)
...
def read_fixed_manifest(path, expected_split):
    """Read and validate one V4B fixed-selection CSV manifest."""

    if not path.is_file():
        raise FileNotFoundError(
            f"The V4B {expected_split} manifest was not found: {path}"
        )

    dataframe = pd.read_csv(path)
    required_columns = {"ImageId", "v4b_label", "v4b_split"}
    ...
    dataframe["ImageId"] = dataframe["ImageId"].astype(str).str.strip()
    dataframe["v4b_split"] = (
        dataframe["v4b_split"].astype(str).str.strip().str.lower()
    )
    dataframe["v4b_label"] = dataframe["v4b_label"].astype(int)

    unexpected_splits = set(dataframe["v4b_split"]) - {expected_split}
    if unexpected_splits:
        raise ValueError(
            f"{path.name} contains unexpected split values: "
            f"{sorted(unexpected_splits)}"
        )

    if dataframe["ImageId"].duplicated().any():
        raise ValueError(f"{path.name} contains duplicate ImageId values.")

    if not set(dataframe["v4b_label"]).issubset({0, 1}):
        raise ValueError(f"{path.name} contains labels other than 0 and 1.")

    return dataframe


def filter_dataset_to_manifest(dataset, dataframe, split_name):
    """
    Keep dataset rows in the exact fixed-manifest order.

    PneumothoraxDataset continues to resolve the DICOM and mask paths in
    exactly the same way as V4A. The Windows DICOMPath text inside the new
    manifest is deliberately not used on Colab.
    """

    row_lookup = {}
    for row in dataset.rows:
        image_id = str(row["ImageId"]).strip()
        if image_id in row_lookup:
            raise ValueError(
                f"The base {split_name} dataset contains duplicate "
                f"ImageId {image_id}."
            )
        row_lookup[image_id] = row

    requested_ids = dataframe["ImageId"].tolist()
    missing_ids = [
        image_id for image_id in requested_ids if image_id not in row_lookup
    ]
    if missing_ids:
        raise ValueError(
            f"{len(missing_ids)} V4B {split_name} IDs were not found in "
            f"PneumothoraxDataset. Examples: {missing_ids[:5]}"
        )

    label_lookup = dict(
        zip(dataframe["ImageId"], dataframe["v4b_label"])
    )
    selected_rows = [row_lookup[image_id] for image_id in requested_ids]

    label_mismatches = []
    for row in selected_rows:
        image_id = str(row["ImageId"]).strip()
        source_label = int(row["HasPneumothorax"])
        manifest_label = int(label_lookup[image_id])
        if source_label != manifest_label:
            label_mismatches.append(image_id)

    if label_mismatches:
        raise ValueError(
            f"V4B label mismatch for {len(label_mismatches)} images. "
            f"Examples: {label_mismatches[:5]}"
        )

    dataset.rows = selected_rows

    positive_count = sum(
        int(row["HasPneumothorax"]) for row in dataset.rows
    )
    negative_count = len(dataset.rows) - positive_count
    positive_fraction = positive_count / len(dataset.rows)

    print(
        f"Fixed {split_name} dataset: {len(dataset.rows):,} images | "
        f"{positive_count:,} positive | {negative_count:,} negative | "
        f"{positive_fraction:.2%} positive"
    )
...
def create_data_loaders(device):
    """Create the fixed V4B training and unchanged V4A validation loaders."""

    training_manifest = read_fixed_manifest(TRAIN_CSV, "train")

    training_base = PneumothoraxDataset(
        split="train",
        image_size=IMAGE_SIZE,
    )
    training_base = filter_dataset_to_manifest(
        training_base,
        training_manifest,
        "training",
    )

    # Deliberately keep the complete original validation split used by V4A.
    # This makes the rotating-versus-fixed training comparison controlled.
    ...
    shuffle_generator.manual_seed(RANDOM_SEED)
...
def print_epoch_results(training, validation):
    positive_correct = (
        validation["positive_samples"]
        - validation["empty_positive_predictions"]
    )
    negative_correct = (
        validation["negative_samples"]
        - validation["false_positive_negative_images"]
    )
    ...
    detection_sensitivity = (
        positive_correct
        / max(validation["positive_samples"], 1)
    )

    detection_specificity = (
        negative_correct
        / max(validation["negative_samples"], 1)
    )
    ...
    print(
        "  Validation overall case-wise Dice: "
        f"{validation['dice']:.6f}"
    )
    print(
        "  Validation image-level detection accuracy: "
        f"{detection_accuracy:.6f}"
    )
    print(
        "  Validation pneumothorax sensitivity: "
        f"{detection_sensitivity:.6f}"
    )
    print(
        "  Validation negative specificity: "
        f"{detection_specificity:.6f}"
    )
    print(
        "  Validation balanced detection accuracy: "
        f"{balanced_detection_accuracy:.6f}"
    )
    print(
    ...
    torch.save(
        {
            ...
            "configuration": {
                ...
                "initialization": (
                    "ImageNet-pretrained ResNet34 encoder with "
                    "randomly initialized decoder"
                ),
                "training_sampling": (
                    "fixed manifest with 45% positive and 55% "
                    "negative image identities; DataLoader shuffle only"
                ),
                "training_manifest": str(TRAIN_CSV),
                "validation_manifest": (
                    "unchanged complete validation split used by V4A"
                ),
...
def main():
    ...
    print("V4B fixed 45/55 fresh 512 x 512 pneumothorax training")
    print("------------------------------------------------")
    ...
    print("Training identities: fixed V4B 45/55 manifest")
    print("Validation: unchanged complete V4A validation split")
    print("Test split: not created or accessed")
    print("Pneumothorax source checkpoint: none")
    ...
    if RESUME_IF_AVAILABLE and LAST_CHECKPOINT_PATH.is_file():
        print(f"Resuming V4B checkpoint: {LAST_CHECKPOINT_PATH}")
        ...
        print(
            "Fresh V4B initialization: ImageNet-pretrained "
            "ResNet34 encoder and a new decoder"
        )
        print("No V3C, V3E, or other pneumothorax checkpoint loaded.")
        print("Training begins at epoch 1.")
    ...
    for epoch in range(start_epoch, MAX_EPOCHS + 1):
        planned_positive_count = sum(
            int(row["HasPneumothorax"])
            for row in training_loader.dataset.rows
        )
        planned_negative_count = len(training_loader.dataset.rows) - (
            planned_positive_count
        )
        planned_positive_fraction = (
            planned_positive_count / len(training_loader.dataset.rows)
        )

        print(f"\nEpoch {epoch}/{MAX_EPOCHS}")
        print(
            "  Fixed training dataset: "
            f"{planned_positive_count:,} positive + "
            f"{planned_negative_count:,} negative = "
            f"{len(training_loader.dataset.rows):,} images "
            f"({planned_positive_fraction:.2%} positive)"
        )
        print("  Fine-tuning phase: decoder + encoder_4 + encoder_3")
    ...
    print("\nV4B fixed 45/55 training finished.")
