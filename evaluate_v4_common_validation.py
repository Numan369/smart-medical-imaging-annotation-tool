"""Evaluate one pneumothorax checkpoint on the full validation split.
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a V4 checkpoint on the complete validation split "
...
def print_results(results):
    print("\nCommon-condition validation results")
    print("-----------------------------------")
...
def main():
    ...
    print("Loading complete validation split only...")
    ...
    output_path = (
        ...
        / f"{arguments.name}_common_validation.json"
    )
